...
The model must answer only from supplied context, preserve server-calculated dates/counts/statuses, state the time/team scope, and acknowledge missing evidence. A low temperature favors repeatable summaries. Gemini returns a fixed title/scope/summary/items/note JSON shape; the server validates it with Zod and the client renders fields directly rather than injecting model HTML. Missing configuration returns `AI assistant is not configured.` without affecting core features. The browser never receives the Gemini key; only Express calls Google.
