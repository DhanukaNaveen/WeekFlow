...
1. **Opening (30s):** Explain the three roles and layered architecture on camera.
2. **Member draft (60s):** Sign in as `member1@example.com`; create a report, add dynamic tasks/hours, save it, reopen, and submit. Point out that it becomes read-only.
3. **Manager review (75s):** Sign out and use `manager@example.com`. Show live metrics/charts, filter reports, open the submitted item, and request changes with a required comment.
